...
class WeekView {
  ...
  render() {
    ...
    container.innerHTML = sessions.map(s => this.renderSession(s)).join('');
  }

  renderSession(s) {
    const dayLabel  = DAY_ABBR[s.day_of_week] || s.day_of_week || '';
    const dateStr   = this.formatDate(s.session_date);
    const focus     = s.focus || 'Training Session';
    const location  = s.location ? `<span class="text-muted" style="font-size:0.8rem;">📍 ${this.cap(s.location)}</span>` : '';
    const duration  = s.estimated_duration ? `<span class="text-muted" style="font-size:0.8rem;">⏱ ${s.estimated_duration} min</span>` : '';
    const rpe       = s.intended_intensity_rpe ? `<span class="text-muted" style="font-size:0.8rem;">RPE ${s.intended_intensity_rpe}</span>` : '';

    const status    = (s.status || '').toLowerCase();
    const isDone    = status === 'completed';
    const statusTag = STATUS_LABEL[status]
      ? `<span style="font-size:0.75rem;color:${isDone ? '#6b7280' : '#92400e'};margin-left:auto;">${STATUS_LABEL[status]}</span>`
      : '';

    const exercises = (s.exercises || []).length
      ? `<div style="margin-top:0.5rem;">
           <p style="font-size:0.75rem;color:#6b7280;margin-bottom:0.25rem;">Main exercises:</p>
           <p style="font-size:0.875rem;color:#374151;">${s.exercises.join(', ')}</p>
         </div>`
      : '';

    const summary = s.session_summary
      ? `<p style="font-size:0.8rem;color:#6b7280;margin-top:0.25rem;">${this.esc(s.session_summary)}</p>`
      : '';

    return `
      <div class="card mb-3" style="${isDone ? 'opacity:0.6;' : ''}">
        <div style="display:flex;align-items:flex-start;gap:0.75rem;">
          ...
          <div style="flex:1;min-width:0;">
            <div style="display:flex;align-items:center;gap:0.5rem;flex-wrap:wrap;">
              <span style="font-weight:600;font-size:0.95rem;">${this.esc(focus)}</span>
              ${statusTag}
            </div>
            ${summary}
